...
std::string var2string(Blob<int> &b) {
  return var2string(std::vector<int>(b.cpu_data(), b.cpu_data() + b.count()));
}


template <typename Dtype>
std::vector<std::pair<std::string, FlyArray> > BaseConvolutionLayer<Dtype>::get_params(
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) { 
    
  std::vector<std::pair<std::string, FlyArray> > parameter2pass;

  parameter2pass.push_back(std::make_pair(
    this->layer_param_.name() + "_weight",
    FlyArray(
             this->blobs_[0]->count(),
  ...
  return parameter2pass;
}


template <typename Dtype>
std::string BaseConvolutionLayer<Dtype>::get_cpu_signature(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  
  std::string layer_name = this->layer_param_.name();
  std::string weight_name = layer_name + "_weight";
  std::string bias_name = (this->bias_term_) ? (layer_name + "_bias") : "NULL";
...
template <typename Dtype>
std::string BaseConvolutionLayer<Dtype>::get_gpu_signature(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  int global_size_x = ((((top[0]->shape(2) * top[0]->shape(3)) - 1) / info.tsn_ + 1)*info.rtsn_);
  int global_size_y = ((((top[0]->shape(1) / info.group_) - 1) / info.tsm_ + 1)*info.rtsm_);
  int global_size_z = (bottom[0]->shape()[0] * info.group_);
  ...
  signature_ss << "\"" << layer_name + "_forward" << "\"" << ", "
               ...
               << "std::vector<int> {" << global_size_x << ", " << global_size_y << ", " << global_size_z << "}";
