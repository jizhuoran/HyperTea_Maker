...
std::vector<std::pair<std::string, FlyArray> > ScaleLayer<Dtype>::get_params(
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) { 
    
  std::vector<std::pair<std::string, FlyArray> > parameter2pass;

  Blob<Dtype>* scale_data = (bottom.size() > 1) ? bottom[1] : this->blobs_[0].get();
  parameter2pass.push_back(std::make_pair(
    this->layer_param_.name() + "_scale",
    FlyArray(
             scale_data->count(),
             scale_data->cpu_data()
    )
  ...
  if (this->bias_layer_) {
    parameter2pass.push_back(std::make_pair(
      ...
      FlyArray(
               this->blobs_[bias_param_id_]->count(),
...
template <typename Dtype>
std::string ScaleLayer<Dtype>::get_gpu_signature(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
std::string ScaleLayer<Dtype>::get_cpu_signature(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
